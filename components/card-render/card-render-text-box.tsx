...
export default function CardRenderTextBox({
  ...
  const cardText = mode === "initial" 
    ? card.initialMode.text 
    : card.anomalyMode.uncommon 
      ? card.anomalyMode.text 
      : "A common anomaly can be manifested into any of the five common anomalies. Light, Storm, Dark, Chaos, or Growth.";
    const cardLoreText = mode === "initial" 
      ? card.initialMode.lore
      : card.anomalyMode.uncommon 
        ? card.anomalyMode.lore 
        : null;

  const bgColorClass50 = mode === "anomaly" 
    ? null : calculateBgColor(card.initialMode.energy_cost as EnergyCost, 50)[0]; 
  const bgColorClass200 = mode === "anomaly" 
    ? null : calculateBgColor(card.initialMode.energy_cost as EnergyCost, 200)[0]; 
